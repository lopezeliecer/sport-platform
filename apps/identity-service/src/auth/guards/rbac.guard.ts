...
import { PERMISSION_KEY, ROLES_KEY, CLUB_CONTEXT_KEY } from '../decorators/permissions.decorator';
import { hasPermission, Module, Action, Scope } from '../rbac/permissions.types';
...
export class RbacGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      this.logger.log(`Acceso autorizado para usuario ${payload.sub} en club ${payload.clubId}`);
  ...
  private async checkOwnResourceAccess(user: JwtPayload, request: any): Promise<boolean> {
  ...
  private async checkClubResourceAccess(user: JwtPayload, request: any): Promise<boolean> {
  ...
  private async checkParentChildAccess(parentId: string, childId: string): Promise<boolean> {
