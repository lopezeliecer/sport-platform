...
import {
  ApiKeyService,
  ApiKey,
} from "../../../../libs/shared/common/src/security/api-key.service";
...
export class ApiKeyController {
  ...
  async getAnalytics(
    ...
    const analytics = this.apiKeyService.getUsageAnalytics(
      keyId,
      analyticsDays
    );
  ...
  async rotateApiKey(@Body() rotateDto: RotateApiKeyDto) {
    ...
    return {
      ...
      warning:
        "Update your services with the new key. The old key has been deactivated.",
    };
  }
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves environment variables for system API keys (for service configuration)",
  })
  @ApiResponse({
    status: 200,
