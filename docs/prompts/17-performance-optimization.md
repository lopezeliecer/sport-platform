...
export class TrainingCalendarComponent implements OnInit, OnDestroy {
  ...
  trackByDay = (index: number, day: CalendarDay): string => day.date.toISOString().split('T')[0];
...
export class TrainingFiltersComponent implements OnInit, OnDestroy {
  ...
  private setupOptimizedControls() {
    ...
    this.athleteControl.valueChanges
...
export class CustomPreloadingStrategy implements PreloadingStrategy {
  ...
  private getDelayForPriority(priority: string): number {
    switch (priority) {
      case 'high':
        return 100; // Precargar casi inmediatamente
      case 'medium':
        return 2000; // Precargar después de 2 segundos
      case 'low':
        return 5000; // Precargar después de 5 segundos
      default:
        return 3000;
    }
  }
...
export class OptimizedUploadService {
  ...
  private async processFile(file: File, options: UploadOptions): Promise<File> {
    ...
    if (this.isImage(file) && file.size > 500 * 1024) {
      // Si > 500KB
      ...
      console.log(
        `Image compressed: ${compressedFile.name} (${this.formatFileSize(compressedFile.size)})`,
      );
  ...
  private chunkedUpload(file: File, options: UploadOptions): Observable<UploadProgress> {
    ...
    return from(chunks).pipe(
      concatMap((chunk, index) => this.uploadChunk(chunk, index, chunks.length, uploadId, options)),
      scan(
        (acc, current) => ({
